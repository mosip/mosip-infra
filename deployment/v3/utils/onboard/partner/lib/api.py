...
class MosipSession:
    ...
    def get_policy_groups(self):
        cookies = {'Authorization' : self.token}
        url = '%s/v1/policymanager/policies/policies/policyGroups' % self.server
    ...
    def add_policy(self, policy_id, name, description, policy, policy_group, policy_type):
        ...
        '''
        url = '%s/v1/policymanager/policies/policies' % self.server
        cookies = {'Authorization' : self.token}
        ts = get_timestamp()
        j = {
            'id': 'string',
    ...
    def update_policy(self, name, description, policy, policy_group, policy_type, policy_id):
        ...
        policy_id: str
        '''
        url = '%s/v1/policymanager/policies/policies/%s' % (self.server, policy_id)
        cookies = {'Authorization' : self.token}
        ts = get_timestamp()
        j = {
            'id': 'string',
    ...
    def add_partner(self, name, contact, address, email, partner_id, partner_type, policy_group):
        url = '%s/v1/partnermanager/partners' % self.server
        cookies = {'Authorization' : self.token}
        ts = get_timestamp()
        j = {
          'id': 'string',
    ...
    def publish_policy(self, policy_group_id, policy_id):
        url = '%s/v1/partnermanager/policies/policies/publishPolicy/policyGroupId/%s/policyId/%s' % (
    ...
    def get_partner_api_key_requests(self, partner_id):
        url = '%s/v1/partnermanager/partners/partners/%s/partnerAPIKeyRequests' % (self.server, partner_id)
        cookies = {'Authorization' : self.token}
        r = requests.get(url, cookies=cookies, verify=self.ssl_verify)
        r = response_to_json(r)
        return r
    ...
    def create_misp(self, org_name, address, contact, email):
        url = '%s/v1/partnermanager/misps/misps' % self.server
        cookies = {'Authorization' : self.token}
        ts = get_timestamp()
        j = {
            'id': 'string',
    ...
    def approve_misp(self, misp_id, status):
        url = '%s/v1/partnermanager/misps/misps/%s/status' % (self.server, misp_id)
        cookies = {'Authorization' : self.token}
        ts = get_timestamp()
        j = {
            'id': 'string',
