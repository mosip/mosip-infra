...
def upload_partner_cert(path, partner_name, partner_id, partner_domain, partner_type):
    session = MosipSession(conf.server, conf.partner_user, conf.partner_pwd, 'partner', ssl_verify=conf.ssl_verify,
                           client_token=False)
    myprint('Uploading partner certificate "%s"' % path)
    cert_data = open(path, 'rt').read()
...
def upload_certs(partner_json):
    j = json.load(open(partner_json, 'rt'))
    myprint('Upload partner CA certificates')
    ca_certs = j['ca_certs']
    for ca_cert in ca_certs: 
      upload_ca_cert(ca_cert, j['partner_domain'])

    myprint('Upload partner certificate')
    upload_partner_cert(j['partner_cert'], j['name'], j['id'], j['partner_domain'], j['partner_type'])
...
def main():
    ...
    try:
        ...
        if args.action == 'upload_certs':
            upload_certs(file_path) # partner jsons
