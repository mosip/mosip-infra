...
function installing_alerting() {
  ...
  ALERTMANAGER_FILE="mosip-infra/deployment/v3/helmsman/utils/alerting/alertmanager.yaml"
  PATCH_CLUSTER_NAME_FILE="mosip-infra/deployment/v3/helmsman/utils/alerting/patch-cluster-name.yaml"
  ...
  kubectl patch secret alertmanager-rancher-monitoring-alertmanager -n $NS  --patch="{\"data\": { \"alertmanager.yaml\": \"$(cat mosip-infra/deployment/v3/helmsman/utils/alerting/alertmanager.yaml |base64 |tr -d '\n' )\" }}"
  ...
  kubectl patch Prometheus rancher-monitoring-prometheus -n $NS --patch-file mosip-infra/deployment/v3/helmsman/utils/alerting/patch-cluster-name.yaml --type=merge
  echo Applying custom alerts
  kubectl apply -f mosip-infra/deployment/v3/helmsman/utils/alerting/custom-alerts/
  return 0
}
