...
function installing_istio_and_httpbin() {
  ...
  kubectl apply -f mosip-infra/deployment/v3/helmsman/utils/global_configmap.yaml
  echo "Installed Global Configmap"

  echo Create ingress gateways, load balancers and istio monitoring
  kubectl apply -f mosip-infra/deployment/v3/helmsman/utils/istio-mesh/nodeport/iop-mosip.yaml
  kubectl apply -f mosip-infra/deployment/v3/helmsman/utils/istio-mesh/nodeport/istio-monitoring/
  ...
  if [[ -n "$public_exists" && -n "$internal_exists" ]]; then
    ...
    helm -n $ISTIO_NS install istio-addons mosip-infra/deployment/v3/helmsman/utils/istio-gateway \
      $gateway_option \
  ...
  kubectl -n $HTTPBIN_NS apply -f mosip-infra/deployment/v3/helmsman/utils/httpbin/svc.yaml
  kubectl -n $HTTPBIN_NS apply -f mosip-infra/deployment/v3/helmsman/utils/httpbin/deployment.yaml
  kubectl -n $HTTPBIN_NS apply -f mosip-infra/deployment/v3/helmsman/utils/httpbin/deployment-busybox-curl.yaml
  kubectl -n $HTTPBIN_NS apply -f mosip-infra/deployment/v3/helmsman/utils/httpbin/vs.yaml
