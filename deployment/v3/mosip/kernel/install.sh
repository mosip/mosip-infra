...
function installing_kernel() {
  ...
  helm -n $NS install authmanager mosip/authmanager --set image.repository=mosipid/kernel-auditmanager-service --set image.tag=1.2.0.1-B2 --version $CHART_VERSION
  ...
  helm -n $NS install pridgenerator mosip/pridgenerator --set image.repository=mosipid/kernel-pridgenerator-service --set image.tag=1.2.0.1-B1 --version $CHART_VERSION

  echo Installing ridgenerator
  helm -n $NS install ridgenerator mosip/ridgenerator --set image.repository=mosipid/kernel-ridgenerator-service --set image.tag=1.2.0.1-B1 --version $CHART_VERSION

  echo Installing syncdata
  helm -n $NS install syncdata mosip/syncdata --set image.repository=mosipid/kernel-syncdata-service --set image.tag=1.2.0.1-B1 --version $CHART_VERSION
