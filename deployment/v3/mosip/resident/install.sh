...
function installing_resident() {
  ...
  helm -n $NS install resident-ui mosip/resident-ui --set resident.apiHost=$API_HOST --set istio.hosts\[0\]=$RESIDENT_HOST --set image.repository=mosipdev/resident-ui --set image.tag=develop --version $CHART_VERSION
