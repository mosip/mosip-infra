...
function installing_admin() {
  ...
  helm -n $NS install admin-hotlist mosip/admin-hotlist --set image.repository=mosipdev/hotlist-service  --version $CHART_VERSION

  echo Installing admin service. Will wait till service gets installed.
  helm -n $NS install admin-service mosip/admin-service --set istio.corsPolicy.allowOrigins\[0\].prefix=https://$ADMIN_HOST image.repository=mosipdev/admin-service --wait --version $CHART_VERSION

  echo Installing admin-ui
  helm -n $NS install admin-ui mosip/admin-ui --set admin.apiUrl=https://$API_HOST/v1/ --set istio.hosts\[0\]=$ADMIN_HOST --set image.repository=mosipdev/admin-ui  --version $CHART_VERSION
