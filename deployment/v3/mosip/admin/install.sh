...
function installing_admin() {
  ...
  helm -n $NS install admin-service mosip/admin-service --set startupProbe.failureThreshold=60 --set istio.corsPolicy.allowOrigins\[0\].prefix=https://$ADMIN_HOST --wait --timeout=10m --version $CHART_VERSION
