...
function installing_captcha() {
  ...
  helm -n $NS install captcha mosip/captcha --set image.repository=mosipdev/captcha-validation-service --set image.tag=develop --version $CHART_VERSION --set metrics.serviceMonitor.enabled=$servicemonitorflag --wait
