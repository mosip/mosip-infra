#!/bin/bash
# Installs dslrig into namespace dsl-dev (alongside existing packetcreator-dev).
...
NS=dsl-dev
CHART_VERSION=1.5.0
# In-cluster URL for packetcreator already deployed in packetcreator-dev
...
function installing_dslrig() {
  ...
  read -p "Please provide packet Utility Base URL (default: $DEFAULT_PACKET_UTILITY_BASE_URL) : " packetUtilityBaseUrl
  if [ -z "$packetUtilityBaseUrl" ]; then
    packetUtilityBaseUrl="$DEFAULT_PACKET_UTILITY_BASE_URL"
  fi
  echo "Using packet Utility Base URL: $packetUtilityBaseUrl"
  ...
  echo Installing dslrig in namespace $NS
  helm -n $NS upgrade --install dslorchestrator mosip/dslorchestrator \
  --set crontime="0 $time * * *" \
