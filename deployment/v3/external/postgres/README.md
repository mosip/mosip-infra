...
3. The script will create a ``db-common-secrets.yaml`` file in the same directory, containing the encoded password as a secret.
4. Then after the yaml file is created run the command:
```
kubectl apply -f db-common-secrets.yaml
```
5. After running the above command ``db-common-secrets`` will be created as ``secrets`` in ``postgres`` namespace.
