  ...
  while true; do
      ...
      if [ $yn = "Y" ]
        then
          ...
          helm -n $NS install postgres-init mosip/postgres-init --set image.repository=mosipdev/postgres-init -f init_values.yaml --version $CHART_VERSION --wait --wait-for-jobs
          break
        else
          break
      fi
