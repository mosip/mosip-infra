  ...
  while true; do
      ...
      if [ $yn = "Y" ]
        then
          ...
          helm -n $NS install postgres-init mosip/postgres-init -f init_values.yaml --image.repository=mosipdev/postgres-init --version $CHART_VERSION --wait --wait-for-jobs
          break
        else
          break
      fi
