...
function installing_keycloak() {
  ...
  echo Installing
  helm -n $NS install $SERVICE_NAME mosip/keycloak --version "7.1.18" --set image.repository=mosipqa/mosip-keycloak --set image.tag=1.2.0.1 --set image.pullPolicy=Always -f values.yaml --wait
